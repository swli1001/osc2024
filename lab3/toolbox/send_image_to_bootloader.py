...
if platform == "linux" or platform == "linux2":
    ...
    with open(args.filename, 'rb') as fd:
        with Serial(args.device, args.baud) as ser:
            ...
            # 将长度信息发送到接收端
            for i in range(8):
                ser.write(struct.pack('<Q', length)[i:i+1])
                ser.flush()
            time.sleep(0.5)

            print("Start sending kernel image by uart1...")
            # 读取文件并发送数据
            fd.seek(0)  # 重新定位到文件的开头
            sent_bytes = 0
            while True:
                chunk = fd.read(1024)  # 每次读取512字节
                if not chunk:
                    break
                ser.write(chunk)
                ser.flush()  # 确保每个块都被发送
                sent_bytes += len(chunk)
                print("{:>6}/{:>6} bytes sent".format(sent_bytes, length))
                time.sleep(0.1)  # 给接收端处理时间
