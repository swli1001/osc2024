...
int get_input_int() {
    ...
    return str_to_int(asize_i);
}

unsigned long get_input_hex() {
    char asize_i[100];
    unsigned int idx = 0;
    char c = '\0';
    while (1) {
...
void parse_cmd()
{
    ...
    else if(str_cmp(buffer, "b_alloc") == 0) {
        uart_send_string("Input decimal allocate page num: ");
        int msize = get_input_int();
        char *ptr = alloc_frame(msize);
        uart_send_string("\r\n");
    }
    else if(str_cmp(buffer, "b_free") == 0) {
        uart_send_string("Input heximal free address: ");
